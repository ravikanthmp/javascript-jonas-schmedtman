...
// CHAPTER 5  Higer order functions
function noisy(f) {
  return (...args) => {
...
noisy(Math.max)(1, 2, 3);

// functions that can even provide some control flow
...
unless( 5 % 2 == 0, () => console.log("halamney moyeh"));

// what if test is replaced by funtion that returns boolean
function doOnlyIfTrue(testFunction, coreFunction, args1, ... args2){
  if( testFunction(args1) ){
    coreFunction(args2);
  }
}

// nothing
doOnlyIfTrue( (... args) => Math.max(args) > 10 , (... args2) => console.log(args2), 11, 2, 3, 4);

[1, 2, 3].forEach( (i) => Math.sqrt(i));

{
  forEach : function abc(fx) {
...
// there is a standard method on arrays called filter that does it 
...
newarr = filter(SCRIPTS, ({living}) => living);
console.log(newarr);
...
// transform each element with a map 
function map(arr, transform){
  newarr = [];
  arr.forEach( (el) => newarr.push(transform(el)));
  return newarr;
}

newarr = map(SCRIPTS, (language) => language.name);

console.log(newarr);

// with the built in method
newarr = SCRIPTS.map( (el) => el.name);
console.log(newarr);

console.log(SCRIPTS.filter( (el) => (el.living)).map( (el) => el.year));

// reduce
function reduce(arr, redfn, start){
  let val = start;
  for(let el of arr){
    val = redfn(el, val);
  }
...
mynums = [1, 2, 3, 4, 5];
console.log(reduce(mynums, (a, b) => a + b, 0));

// charecter count
function charCount(a){
 return a.ranges.reduce( (count, [from, to]) => count + (to - from + 1), 0);
}

// max chars
function calcMax(scripts){
  return scripts.reduce( (a, b) => charCount(a) >= charCount(b) ? a : b);
}

console.log(calcMax(SCRIPTS));
...
console.log(average(SCRIPTS.filter( (el) => el.living).map( (el) => el.year)));
console.log(average(SCRIPTS.filter( (el) => !el.living).map( (el) => el.year)));

// some
function within(scripts, codepoint){
   for(let sc of scripts){
     if (sc.ranges.some( ([start, end]) => start <= codepoint && codepoint <= end)){
       return sc;
     }
   }
...
console.log(within(SCRIPTS, 121))